...
const STEPS = [
  { key: "create", label: "创建应用", desc: "在飞书开放平台创建一个企业自建应用。进入「应用管理」→「创建应用」，选择「企业自建应用」。" },
  { key: "bot", label: "开启机器人", desc: "在应用的「添加应用能力」中，添加「机器人」能力。这样应用才可以在飞书中作为 Bot 发消息。" },
  { key: "event", label: "配置事件订阅", desc: "进入「事件与回调」→「事件订阅」，选择「使用长连接接收事件」（WebSocket 模式，无需公网 webhook）。然后添加事件 im.message.receive_v1。" },
  { key: "publish", label: "发布应用", desc: "在「版本管理与发布」中创建应用版本并提交发布。如果是开发环境，可以先发布到测试企业。" },
  { key: "credentials", label: "填入凭证", desc: "将飞书后台的 App ID 和 App Secret 填入下方。" },
  { key: "test", label: "连接测试", desc: "验证飞书凭证是否正确，确认 WebSocket 连接正常。" },
];
    ...
    <div className="flex gap-4 h-full">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="bg-[#EFF6FF] rounded-2xl p-4 mb-4">
          <p className="text-[11px] text-neutral-500 leading-5">{step.desc}</p>
        </div>
