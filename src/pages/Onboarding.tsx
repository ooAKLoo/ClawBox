...
export default function Onboarding({ onComplete }: OnboardingProps) {
  ...
  const goNext = async () => {
    // Auto-save when leaving config steps
    const currentStep = STEPS[stepIndex];
    const api = window.clawbox;
    if (api) {
      if (currentStep === "apiKey" && apiKey.trim()) {
        ...
        <div className="p-6 pb-0">
          ...
          <div className="flex gap-1 mb-6">
            {STEPS.map((_, i) => (
              <div
                key={i}
                className={`h-1 flex-1 rounded-full transition-colors duration-300 ${i <= stepIndex ? "bg-neutral-800" : "bg-neutral-200"
                  }`}
              />
            ))}
          </div>
              ...
              {step === "welcome" && (
                <div className="py-8 text-center">
                  ...
                  <div className="text-[11px] text-neutral-500 leading-5 max-w-md mx-auto">
                    OpenClaw 桌面客户端。接下来的几个步骤将帮助你完成环境配置、模型接入和安全设置。
                  </div>
                </div>
              )}
                ...
                <div className="space-y-2">
                  {providers.map((p) => (
                    <motion.button
                      ...
                      className={`w-full text-left rounded-xl p-3 flex items-center justify-between transition-colors duration-200 ${selectedProvider.id === p.id
                    ...
                    >
                      ...
                      {p.tag && (
                        <span className={`text-[9px] font-medium px-2 py-0.5 rounded-lg ${selectedProvider.id === p.id
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    <motion.button
                      ...
                      className={`text-[10px] font-medium px-4 py-2 rounded-lg ${apiKey.trim() && !modelTesting
                          ? "bg-neutral-800 text-white"
                          : "bg-neutral-200 text-neutral-400"
                        }`}
                    >
          ...
          <div className="flex gap-2">
            {step === "launch" ? (
              <motion.button
                ...
                onClick={handleFinish}
                disabled={!launched}
                className={`text-[10px] font-medium px-4 py-2 rounded-lg ${launched
                    ? "bg-neutral-800 text-white"
                    : "bg-neutral-200 text-neutral-400"
                  }`}
              >
              ...
              <motion.button
                ...
                onClick={goNext}
                disabled={!canProceed()}
                className={`text-[10px] font-medium px-4 py-2 rounded-lg ${canProceed()
                    ? "bg-neutral-800 text-white"
                    : "bg-neutral-200 text-neutral-400"
                  }`}
              >
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={`text-[9px] mt-0.5 ${step.status === "error" ? "text-red-400" :
